...
class VehicleManager:
    def __init__(
        ...
        self._hydrate_token_credentials()
        self.vehicles: dict = {}
    ...
    def _hydrate_token_credentials(self) -> None:
        """Attach config-entry credentials only for the lifetime of this manager."""
        if self.token is not None:
            self.token.username = self.username
    ...
    def get_implementation_by_region_brand(
        ...
        if REGIONS.get(region) != REGION_EUROPE or BRANDS.get(brand) != BRAND_KIA:
            raise APIError("Only Kia Connect Europe is supported")
