...
from .logging_utils import SafeDebugLogger, debug_event
...
CCI_UPSTREAM_BASELINE: str = "v4.29.1"
...
class KiaUvoApiEU(ApiImplType1):
    ...
    def __init__(self, region: int, brand: int, language: str) -> None:
        ...
        if BRANDS[self.brand] != BRAND_KIA:
            raise ValueError("Only Kia Connect EU is supported")
        ...
        # OneApp/CCI login flow used by Kia Connect EU.
        ...
        self._cci_client_os_version: str = "27"
        self._cci_notification_provider: str = "IOS_APPSTORE"
    ...
    def _login_with_password_cci(
        ...
        """OneApp/CCI password login for Kia Connect EU.
        ...
        if resp.status_code != 302:
            raise AuthenticationError(
                f"Signin failed: HTTP {resp.status_code}. "
    ...
    def _exchange_ccs_token(
        ...
        if not ccs_token:
            raise AuthenticationError(
                "CCS token exchange returned no access token"
            )
    ...
    def _refresh_cci_token(self, token: Token) -> Token:
        ...
        if resp.status_code != 200:
            raise AuthenticationError(
                f"CCI token refresh failed: HTTP {resp.status_code}"
            )
        data = resp.json()
    ...
    def update_vehicle_with_cached_state(self, token: Token, vehicle: Vehicle) -> None:
        ...
        debug_event(_LOGGER, "cached_vehicle_status")
    ...
    def _force_refresh_vehicle_state_ccs2(self, token: Token, vehicle: Vehicle) -> None:
        ...
        debug_event(_LOGGER, "force_refresh_ccs2")
        _check_response_for_errors(response)
        state = response["resMsg"]["state"]["Vehicle"]
        self._update_vehicle_properties_ccs2(vehicle, state)
        self._set_cached_location_park(token, vehicle)
    ...
    def _set_cached_location_park(self, token: Token, vehicle: Vehicle) -> None:
        ...
        try:
            ...
            debug_event(_LOGGER, "location_lookup")
            _check_response_for_errors(response)
    ...
    def _get_forced_vehicle_state(self, token: Token, vehicle: Vehicle) -> dict:
        ...
        debug_event(_LOGGER, "forced_vehicle_data")
        _check_response_for_errors(response)
        mapped_response = {}
    ...
    def charge_port_action(
        ...
        debug_event(_LOGGER, "charge_port_action")
    ...
    def _get_charge_limits(self, token: Token, vehicle: Vehicle) -> dict:
        ...
        debug_event(_LOGGER, "charging_limits")
        _check_response_for_errors(response)
    ...
    def _get_driving_info(self, token: Token, vehicle: Vehicle) -> dict:
        ...
        debug_event(_LOGGER, "driving_info_all_time")
        ...
        debug_event(_LOGGER, "driving_info_30d")
    ...
    def valet_mode_action(
        ...
        payload = {"action": action.value}
        debug_event(_LOGGER, "valet_mode_action")
        response = self.session.post(
            url, json=payload, headers=self._get_control_headers(token, vehicle)
        ).json()
        debug_event(_LOGGER, "valet_mode_action")
    ...
    def _get_device_id(self, stamp: str):
        ...
        debug_event(_LOGGER, "device_registration")
    ...
    def _get_cookies(self) -> dict:
        ...
        debug_event(_LOGGER, "cookie_bootstrap")
