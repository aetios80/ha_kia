...
def strip_token_credentials(token_data: Any) -> dict[str, Any] | None:
    """Remove legacy credential copies from a persisted token dictionary."""
    if not isinstance(token_data, dict):
        return None
    return {
        key: value
        for key, value in token_data.items()
        if key not in {"username", "password", "pin"}
    }
